...
export async function POST(request:NextRequest,{params}:{params:Promise<{token:string}>}){
  ...
  const form=await request.formData(),signerName=String(form.get("signerName")||"").trim(),consent=form.get("consent"),signature=form.get("signature"),front=form.get("idFront"),back=form.get("idBack");
  if(signerName.length<3||consent!=="true"||!(signature instanceof File)||!(front instanceof File)||!(back instanceof File))return NextResponse.json({error:"Completa la firma, aceptación y ambas caras del documento"},{status:400});
  for(const file of [signature,front,back])if(!["image/png","image/jpeg","image/webp"].includes(file.type)||file.size>8*1024*1024)return NextResponse.json({error:"Las evidencias deben ser imágenes PNG, JPG o WebP de hasta 8 MB"},{status:415});
  const signatureBytes=new Uint8Array(await signature.arrayBuffer()),frontBytes=new Uint8Array(await front.arrayBuffer()),backBytes=new Uint8Array(await back.arrayBuffer()),prefix=`contracts/${a.id}`,keys={signatureKey:`${prefix}/signature.png`,idFrontKey:`${prefix}/id-front-${crypto.randomUUID()}`,idBackKey:`${prefix}/id-back-${crypto.randomUUID()}`},s3=storageClient();
  await Promise.all([[signature,signatureBytes,keys.signatureKey],[front,frontBytes,keys.idFrontKey],[back,backBytes,keys.idBackKey]].map(async([value,bytes,key])=>{const file=value as File;await s3.send(new PutObjectCommand({Bucket:storageBucket(),Key:key as string,Body:bytes as Uint8Array,ContentType:file.type}))}));
