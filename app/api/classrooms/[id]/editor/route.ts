...
async function resolveTarget(
  ...
  if (topicId) {
    const topic = await prisma.topic.findFirst({
      where: { id: topicId, lesson: { module: { classroomId } } },
  ...
  const lesson = await prisma.lesson.findFirst({
    where: { id: lessonId, module: { classroomId } },
    select: { id: true },
  });
        ...
        include: {
          lessons: {
            ...
            include: {
              topics: {
                orderBy: { position: "asc" },
                include: { resources: true, activities: true },
              },
              resources: { where: { topicId: null } },
              activities: { where: { topicId: null } },
            },
          },
        },
      },
...
export async function POST(
  ...
  try {
    ...
    if (body.action === "division") {
      const position =
        (await prisma.module.count({ where: { classroomId: id } })) + 1;
      await prisma.module.create({
        data: {
          classroomId: id,
          title: titleSchema.parse(body.title),
          position,
          published: true,
        },
      });
      ...
      const module = await prisma.module.findFirst({
        where: { id: String(body.parentId), classroomId: id },
      });
      if (!module) throw new Error("Unidad inválida");
      const position =
        (await prisma.lesson.count({ where: { moduleId: module.id } })) + 1;
      ...
      const target = await resolveTarget(
        id,
        body.lessonId && String(body.lessonId),
        body.topicId && String(body.topicId),
      );
      ...
      if (["SUBTOPIC", "TEXT", "PAGE", "DIVIDER"].includes(body.elementType)) {
        const position =
          (await prisma.topic.count({ where: { lessonId: target.lessonId } })) +
          1;
        await prisma.topic.create({
          data: { lessonId: target.lessonId, title, description, position },
        });
        ...
        resourceTypes.includes(body.elementType) ||
        body.elementType in resourceAliases
      ) {
        const url = z.string().trim().min(1).max(4000).parse(body.url);
        const type =
          body.elementType in resourceAliases
            ? resourceAliases[body.elementType as keyof typeof resourceAliases]
            : body.elementType;
      ...
      if (kind === "module") {
        const item = await prisma.module.findFirst({
          where: { id: elementId, classroomId: id },
        });
        if (!item) throw new Error("Unidad inválida");
        ...
        const item = await prisma.lesson.findFirst({
          where: { id: elementId, module: { classroomId: id } },
        });
        if (!item) throw new Error("Tema inválido");
        await prisma.lesson.update({
          where: { id: elementId },
          data: { title, content: { text: description } },
        });
      } else if (kind === "topic") {
        const item = await prisma.topic.findFirst({
          where: { id: elementId, lesson: { module: { classroomId: id } } },
        });
        if (!item) throw new Error("Subtema inválido");
      ...
      if (kind === "topic") {
        ...
        await prisma.$transaction([
          prisma.topic.update({
            where: { id: elementId },
            data: { lessonId: target.lessonId, position },
          }),
          prisma.resource.updateMany({
            where: { topicId: elementId },
            data: { lessonId: target.lessonId },
          }),
    ...
    return NextResponse.json({ ok: true, activityId: createdActivityId });
  } catch (error) {
    console.error("course_editor_error", error);
    return NextResponse.json(
      { error: "No se pudo guardar el cambio. Revisa los datos." },
      { status: 400 },
    );
  }
}
