...
export async function GET(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  const classroom = await prisma.classroom.findUnique({
    ...
    include: {
      ...
      modules: { orderBy: { position: "asc" }, include: { lessons: { orderBy: { position: "asc" }, include: { topics: { orderBy: { position: "asc" } }, resources: true, progress: user?.role === "STUDENT" ? { where: { studentId: user.sub }, select: { completedAt: true } } : false, activities: { include: { submissions: user?.role === "STUDENT" ? { where: { studentId: user.sub }, include: { student: { select: { id: true, name: true, email: true } } } } : { include: { student: { select: { id: true, name: true, email: true } } } } } } } } } },
    },
  });
