...
export async function POST(request:NextRequest,context:{params:Promise<{id:string}>}) {
  ...
  try {
    ...
    else if(action==="activity") { const lesson=await prisma.lesson.findFirst({where:{id:String(body.lessonId),module:{classroomId:id}}}); if(!lesson)throw new Error("Lección inválida"); await prisma.activity.create({data:{title:String(body.title).trim(),description:String(body.description||""),type:body.type||"ASSIGNMENT",dueAt:body.dueAt?new Date(body.dueAt):null,maxScore:Number(body.maxScore||100),lessonId:lesson.id}}); }
    else if(action==="announcement") await prisma.announcement.create({data:{title:String(body.title).trim(),body:String(body.body).trim(),classroomId:id}});
    ...
    else if(action==="grade") { const submission=await prisma.submission.findFirst({where:{id:String(body.submissionId),activity:{lesson:{module:{classroomId:id}}}},select:{id:true}}); if(!submission)throw new Error("Entrega inválida"); const score=Math.max(0,Number(body.score)); await prisma.submission.update({where:{id:submission.id},data:{score,feedback:String(body.feedback||""),gradedAt:new Date()}}); }
